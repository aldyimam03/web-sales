<x-app :pageTitle="$pageTitle">
    ...
    <div class="max-w-6xl mx-auto">
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-slate-800 mb-2">Daftar Pembayaran</h1>
        ...
        <!-- Filter -->
        <form method="GET" action="{{ route('payments.index') }}" class="mb-6 flex flex-wrap gap-4 items-end">
            <div>
                ...
                <table id="payments-table" class="min-w-full text-sm">
                    <thead>
                        <tr class="bg-slate-200 text-slate-700 uppercase text-xs">
                            ...
                            <th class="px-6 py-3 text-center">Penjualan</th>
                    ...
                    <tbody>
                        @forelse ($payments as $payment)
                            @php
                                $totalBayar = $payment->sale->payments()->sum('jumlah');
                                $sisaTagihan = $payment->sale->total_harga - $totalBayar;
                            @endphp
                            <tr>
                                <td class="px-6 py-4 text-center">
                                    {{ $loop->iteration + $payments->perPage() * ($payments->currentPage() - 1) }}</td>
                                ...
                                <td class="px-6 py-4 text-center">{{ $payment->sale->kode_penjualan }}</td>
                                <td class="px-6 py-4 text-center space-x-2">
                                    ...
                                    <form action="{{ route('payments.destroy', $payment->id) }}" method="POST"
                                        class="inline-block" onsubmit="return confirm('Hapus pembayaran ini?');">
                                        @csrf @method('DELETE')
                                        <button type="submit"
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function() {
                var columnCount = $('#payments-table thead tr th').length;
